/**
 * convert_topic_kjv_to_id.mjs
 * Fetches all "Topic:" KJV published sets from PartyKit,
 * converts each verse to Indonesian (Terjemahan Baru) via bolls.life,
 * then publishes the Indonesian version back to PartyKit.
 *
 * Run: node src/convert_topic_kjv_to_id.mjs
 */
...
console.log(`Found ${kjvTopics.length} KJV Topic: sets to convert to Indonesian\n`);

for (const set of kjvTopics) {
  const newId = set.id.replace(/-kjv$/, '-id');
  console.log(`\n[${newId}] "${set.title}" — ${set.verses.length} verses`);

  const alreadyExists = allSets.find(s => s.id === newId);
  if (alreadyExists) console.log(`  ⚠ Indonesian version already exists, overwriting...`);
  ...
  for (const v of set.verses) {
    ...
    if (text) {
      ...
      newVerses.push({
        id: (v.id || '').replace(/-kjv$/g, '-id').replace(/-kjv-/g, '-id-'),
        reference: idRef,
        title: v.title,
        text,
      });
...
console.log('\n✅ All Topic: Indonesian sets converted and published.');
